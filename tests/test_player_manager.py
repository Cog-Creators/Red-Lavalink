...
@pytest.mark.asyncio
async def test_autoconnect(initialize_lavalink, voice_channel, voice_server_update, voice_state_update):
    server = voice_server_update()
    state = voice_state_update()
    await lavalink.player_manager.on_socket_response(server)

    assert voice_channel.guild.id not in set(id for _, ids in lavalink.node._nodes.items() for id in ids)
